...
try {
    ...
    // Modified SQL to fetch 'image' from 'products' table
    $select_sql = "
        SELECT p.id AS product_id, p.code, p.name, p.description, p.quantity, p.Price, p.availability, p.category_id, 
               c.name AS category_name, 
               p.image AS product_image  -- Using 'image' column from 'products' table
        FROM products p
        LEFT JOIN categories c ON p.category_id = c.id
        WHERE c.name = 'Electronics'  -- Filter for Electronics category
    ";
